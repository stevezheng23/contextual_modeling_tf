        ...
        with tf.variable_scope("representation", reuse=tf.AUTO_REUSE):
            ...
            if word_feat_enable == True:
                ...
                word_feat_layer = WordFeat(vocab_size=word_vocab_size, embed_dim=word_embed_dim, dropout=word_dropout,
                    pretrained=word_embed_pretrained, regularizer=self.regularizer, random_seed=self.random_seed, trainable=word_feat_trainable)
...
class AttentiveModule(object):
    ...
    def __call__(self,
        ...
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            ...
            output_module, output_module_mask = self.dense_layer(input_attention, input_attention_mask)
        ...
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            ...
            for i in range(self.num_layer):
                ...
                module_layer = AttentiveModule(num_head=self.num_head, unit_dim=self.unit_dim, activation=self.activation,
                    dropout=self.dropout, att_dropout=self.att_dropout, layer_dropout=layer_dropout, is_self=True,
